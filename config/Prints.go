...
// GetPrint  结果显示
func GetPrint(respCode int,Bodylen string ,body []byte,url string,path string,Rurl string) {

	if respCode >= 200 && respCode < 300 && Thundred(body) == true {
		Urlpath := url+path
	...
	} else if respCode >= 400 && respCode < 500  && Thundreds(body) == true{
		if respCode == 403 {
	...
	} else if respCode >= 500 && respCode < 600 && Fhundreds(body) == true {
